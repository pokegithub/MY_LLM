...
def run_agent_backend_provision_small_candidate(args):
    from agent.backend import provision_small_candidate_model

    payload = provision_small_candidate_model(
        model_id=args.small_candidate_model_id,
        destination=args.small_candidate_dest,
        config_path=args.small_candidate_config,
    )
    if OUTPUT_JSON:
        _emit_json(payload)
        return
...
def run_hidden_eval_seed_run(args):
    ...
    print(f"  retrieval_partial   : {counts.get('retrieval_partial_answers')}")
    ...
    print(f"  retrieval_claims_ok : {counts.get('retrieval_claims_supported')}")
    print(f"  retrieval_claims_partial: {counts.get('retrieval_claims_partial')}")
    print(f"  claims_unsupported  : {counts.get('claims_unsupported')}")
    print(f"  claims_contradicted : {counts.get('retrieval_claims_contradicted')}")
    print(f"  missing_citations   : {counts.get('retrieval_missing_citations')}")
    print(f"  contradiction_failures: {counts.get('retrieval_contradiction_failures')}")
...
def run_retrieval_answer(args):
    ...
    print(f"  claims_partial : {validation.get('claims_partially_supported')}")
    print(f"  claims_unsupported: {validation.get('claims_unsupported')}")
    print(f"  claims_contradicted: {validation.get('claims_contradicted')}")
    print(f"  missing_citations: {validation.get('claims_missing_citations')}")
    print(f"  support_quality: {validation.get('answer_support_quality')}")
...
Commands:
  ...
  status       Show pipeline status
    ...
    parser.add_argument(
        "--small-candidate-model-id",
        default=None,
        help="Optional allowlisted small instruction model id for agent-backend-provision-small-candidate.",
    )
    parser.add_argument(
        "--small-candidate-dest",
        default=None,
        help="Optional local destination for agent-backend-provision-small-candidate.",
    )
    parser.add_argument(
        "--small-candidate-config",
        default="./configs/backend_smoke_small_candidate.json",
        help="Config path written for the provisioned small candidate backend smoke.",
    )
    parser.add_argument(
        "--candidate-id",
